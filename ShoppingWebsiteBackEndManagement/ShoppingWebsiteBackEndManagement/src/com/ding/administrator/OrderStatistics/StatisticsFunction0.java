...
public class StatisticsFunction0 {
	...
	*/
	...
	public JTable getTableForFunction0() throws Exception {
		...
		String sql = "select storeNo, count(distinct orderNo), cast(sum(itemMoney) as decimal(10, 2)) from order_item  where orderNo in (select orderNo from ShoppingWebsiteBackEndDB.`order` where status != 'Cancelled' and createDate between '" + this.startDate + "' and '" + this.endDate + "') group by storeNo order by count(orderNo) desc";
		Object[][] rowData;
