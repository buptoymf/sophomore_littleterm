...
public class SearchStore {
	...
	public void build() {
		...
		continueButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				tablePanel.repaint();
				try {
					table = getTableForStore("select * from store where name like '%" + text.getText() + "%'");
				} catch (Exception e1) {
		...
		try {
			table = this.getTableForStore(sql);
			scrollPane = new JScrollPane(table);
			tablePanel.add(scrollPane);
		} catch (Exception e1) {
			// TODO Auto-generated catch block
	...
	public JTable getTableForStore(String sql) throws Exception {
		ResultSet result;
		ResultSetMetaData rsmd;
		//String sql = "select * from product";
		Object[][] rowData;
		Object[] columnNames = {"storeNo", "name", "type", "service hotline", "found date", "status"};
